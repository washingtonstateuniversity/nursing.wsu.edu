...
jQuery(document).ready(function($) {
	$('.builder-banner-slide').each(function() {
		var block = $(this);
		var showTitle = block.Next('.builder-banner-slide').find('<h1>').text();

		block.find('<h1>').html('<span class="next-title">' . + . showTitle . + '</span>');
		console.log(showTitle);
		/*document.getElementById("next-title").innerHTML = showTitle;*/
	});
});



$.each('.builder-banner-slide', function() {

	var block = $(this);
	var title = block.Next('.builder-banner-slide').find('<h1>').text();
		block.find('<h1>').html('<span class="next-title">' . + . title . + '</span>');
		console.log(title);

	$('.builder-banner-slide').last();
		var firstTitle = '.builder-banner-slide';
		$('.builder-banner-slide : eq(0)').html('<span class="next-title">' . + . firstTitle . + '</span>');
});
</script>
...
<section id="<?php echo esc_attr( $section_id ); ?>" class="row single builder-section <?php echo $section_classes; ?> <?php echo esc_attr( ttfmake_builder_get_banner_class( $ttfmake_section_data, $ttfmake_sections ) ); ?>">
	<div class="column one <?php echo esc_attr( $column_classes ); ?>">
		...
		<div class="builder-section-content<?php echo ( $is_slider ) ? ' cycle-slideshow' : ''; ?>"<?php echo ( $is_slider ) ? ttfmake_builder_get_banner_slider_atts( $ttfmake_section_data ) : ''; ?> data-cycle-prev=".cycle-prev" data-cycle-next=".cycle-next">
				...
				<div class="builder-banner-slide<?php echo ttfmake_builder_banner_slide_class( $slide ); ?>" style="<?php echo ttfmake_builder_banner_slide_style( $slide, $ttfmake_section_data ); ?>">
					<div class="builder-banner-content">
						<div class="builder-banner-inner-content">
							...
							<div class="inquirebuttons">
								<a href="<?php echo esc_url( $slide['slide-url'] ); ?>">Read Story</a>
							</div>
							
							<?php if ( $is_slider && false === (bool) $ttfmake_section_data['hide-arrows'] ) : ?>
							<div class="cycle-prev"></div>
							<div class="cycle-next"><p><span class="next-text">Next /</span> <span id="next-title">Dummy text to style</span></p></div>
							<?php endif; ?>
						</div>
					</div>
